...
def status_for_real_yield(value: float) -> str:
    if value >= 2.5:
        return "red"
    if value >= 2.0:
        return "amber"
...
def transform_fred_real_yield(payload: dict[str, Any]) -> list[SignalObservation]:
    ...
    for item in observations:
        ...
        rows.append(
            SignalObservation(
                ...
                metric=METRIC,
                value=value,
                status=status_for_real_yield(value),
                source=SOURCE,
                raw_payload={"series_id": SERIES_ID, "date": raw_date, "value": raw_value, "units": "percent"},
            )
        )
...
def fetch_observations() -> list[SignalObservation]:
    ...
    return transform_fred_real_yield(get_json(fred_observations_url(settings.fred_api_key)))
